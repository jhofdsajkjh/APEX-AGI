...
impl OpenHumanApiClient {
    ...
    pub fn create_agent_request(&self, agent_id: &str, prompt: &str) -> OpenHumanMessage {
...
impl OpenHumanWorkflowExecutor {
    ...
    pub async fn execute_workflow(&self, workflow_id: &str) -> Result<HashMap<String, String>> {
        ...
        let workflow =
            workflow.ok_or_else(|| anyhow::anyhow!("Workflow not found: {}", workflow_id))?;
        ...
        for node in &workflow.nodes {
            ...
            results.insert(
                node.node_id.clone(),
                format!("completed:{}", node.node_type),
            );
        }
...
pub trait OpenHumanAdapterTrait: Send + Sync {
    ...
    fn send_agent_request(
        &self,
        message: OpenHumanMessage,
    ) -> impl std::future::Future<Output = Result<OpenHumanMessage>> + Send;

    /// Execute workflow
    fn execute_workflow(
        &self,
        workflow: OpenHumanWorkflow,
...
impl OpenHumanAdapterTrait for OpenHumanAdapter {
    ...
    async fn execute_workflow(
        ...
        self.workflow_executor
            ...
            .await?;
        self.workflow_executor
            .execute_workflow(&workflow.workflow_id)
            .await
    }
